...
import com.fosung.gui.ZDlgAsyncProgress;
import com.fosung.gui.ZDlgProgress;
...
public class OtherViewActivity extends BaseSecondLevelActivity {
    ...
    private void showDlgAsyncProgress() {
        final ZDlgAsyncProgress dlg = new ZDlgAsyncProgress(mActivity);
        dlg.setDoInterface(new ZDlgAsyncProgress.DoInterface() {
            @Override
            public ZDlgAsyncProgress.ProcessInfo onDoInback() {
                SystemClock.sleep(1000);
                dlg.setMessageInBack("执行第二步");
                SystemClock.sleep(2000);

                ZDlgAsyncProgress.ProcessInfo info = new ZDlgAsyncProgress.ProcessInfo();
                info.setMsg("执行结果：success");
                return info;
            }

            @Override
            public void onPostExecute(ZDlgAsyncProgress.ProcessInfo info) {
    ...
    private void showDlgProgress() {
        final ZDlgProgress progress = new ZDlgProgress(mActivity);
